...
async function handleSubmit() {
    const { error } = await useAPI('/meals', {
        method: 'post',
        query: { userId },
        body: {
    ...
    if (error.value) {
        toast.add({ severity: 'error', summary: 'Erro ao adicionar refeição', detail: 'Algo deu errado. Tente novamente', life: 3000 })
    } else {
        toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Refeição registrada com sucesso', life: 3000 })
...
<template>
    <form class="flex gap-4 w-full items-end" @submit.prevent="handleSubmit">
        ...
        <div class="flex flex-col gap-2 w-4/12">
            ...
            <Select v-model="mealType" id="mealType" :options="mealTypeOpts" option-label="label" option-value="value"
                fluid placeholder="Selecione um tipo de refeição" />
        </div>
        <div class="flex flex-col gap-2 w-6/12">
            <label for="description">Descrição</label>
            <InputText v-model="description" id="description" fluid placeholder="Adicione uma breve descrição" />
        </div>
        <Button type="submit" class="w-2/12">Salvar</Button>
    </form>
</template>